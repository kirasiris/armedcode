...
const Single = ({ auth = {}, object = {} }) => {
	...
	useEffect(() => {
		const checkEnrollment = async () => {
			...
			const res = await fetchurl(
				`/subscribers?user=${auth.data._id}&resourceId=${object._id}&onModel=Membership`,
				"GET",
				"no-cache"
			);
			console.log("Verifying user enrollment", res);
	...
	// 					(object?.data?.isFree && !enrollmentVerification?.sucess && (
	...
	// 						enrollmentVerification?.sucess && <p>Already enrolled</p>)
	// 				) : (
	// 					<a
